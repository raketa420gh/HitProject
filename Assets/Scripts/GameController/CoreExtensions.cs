...
public static class CoreExtensions
{
    ...
    public static void SetAlpha(this UnityEngine.UI.Image image, float aValue)
    {
        image.color = image.color.SetAlpha(aValue);
    }
    ...
    {
        return string.Format("#{0:X2}{1:X2}{2:X2}", (byte)(Mathf.Clamp01(color.r) * 255),
            (byte)(Mathf.Clamp01(color.g) * 255), (byte)(Mathf.Clamp01(color.b) * 255));
    }
    ...
    private static readonly BindingFlags PUBLIC_BINDING_FLAGS =
                        ...
                        if (list != null)
                        {
                            ...
                            for (int j = 0; j < listCount; j++)
                            {
                                ...
                                for (int f = 0; f < arrayFieldInfos.Length; f++)
                                {
                                    ...
                                    stringBuilder.AppendLine(space + "    " + arrayFieldInfos[f].Name + " - " +
                                                             (arrayValue != null ? arrayValue.ToString() : "NULL"));
                                }
                            }
                        }
                        else
